...
<section class="boxes">
    <div>
        <div class="contain">
            ...
            <?php
            $query = 'SELECT * FROM ( SELECT * FROM pjsUpload ORDER BY date DESC LIMIT 3) as r ORDER BY date DESC';
...
<div class="sermons-wrapper">
    <div class="row">
        ...
        <?php
        $countQuery = "SELECT COUNT(*) AS count FROM `pjsUpload`";
        $countResult = mysqli_query($conn, $countQuery);
        while ($row = mysqli_fetch_assoc($countResult)) {
            $countOutput = $row['count'];
        }

        $codeQuery = $mysqli->query("SELECT * FROM `pjsUpload` ORDER BY vid_title ASC");
